...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    case MB28:
        if (record->event.pressed) {
            // when keycode MB28 is pressed
            for(int i=1; i<=28; i++) {
                tap_code_delay(KC_SPC, 30);
                _delay_ms(30);
            }
        } else {
            // when keycode MB28 is released
        }
        break;
    }
    return true;
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [2] = LAYOUT_60_iso_arrow(
        KC_KB_POWER, _______, _______,     MB3,     MB4,     MB5,     MB6,     MB7, KC_PAST,     MB9,    MB10, KC_PMNS, KC_PPLS,    MB28,
        ___________, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______,
